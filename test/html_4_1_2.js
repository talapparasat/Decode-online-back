...
describe('First test: ', async () => {
    ...
    it('3.Проверка высоты и ширины',  function () {
        ...
        expect(dom.window.document.getElementsByTagName('video')[0].getAttribute('width')).to.equal("800");
    });
    it('4.Проверка источника',  function () {
        expect(dom.window.document.getElementsByTagName('video')[0].getAttribute('src')).to.equal("featured.mp4");
        expect(dom.window.document.getElementsByTagName('source')[0]).not.to.exist;
    });
    it('5.Проверка воспроизведения',  function () {
        expect(dom.window.document.getElementsByTagName('video')[0].getAttribute('autoplay')).to.exist;
        expect(dom.window.document.getElementsByTagName('video')[0].getAttribute('muted')).to.exist;
        expect(dom.window.document.getElementsByTagName('video')[0].getAttribute('loop')).to.exist;
    });
